...
///
/// Tilde and env expansion happen here rather than in a per-platform
/// layer, so behaviour is consistent cross-platform:
///
/// 1. `~/x` and `~` expand via `dirs::home_dir()`.
/// 2. `$VAR/x` expands via `std::env::var`.
/// 3. Strings starting with a known URI scheme are rejected up front so the
///    OS opener routes them as URLs (saves one filesystem syscall).
