...
impl WindowState {
    ...
    pub fn frame_click(
        ...
        match action {
            ...
            FrameAction::ShowMenu(x, y) => {
                self.window.show_window_menu(seat, serial, (x, y))
            }
            _ => (),
        };
    ...
    fn set_cursor_grab_inner(
        ...
        let old_mode =
            std::mem::replace(&mut self.cursor_grab_mode.current_grab_mode, mode);
